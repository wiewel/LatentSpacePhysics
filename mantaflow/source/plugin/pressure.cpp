...
// *****************************************************************************
// Adjustments and new functions for pressure prediction
// *****************************************************************************

// *****************************************************************************
// KERNELS
KERNEL(bnd = 1)
void ComputePressureGhostFluid(MACGrid &vel, const FlagGrid &flags, const Grid<Real> &pressure, Grid<Real> &pnew, const Grid<Real> &phi, Real gfClamp, const Grid<Real> *curv, const Real surfTens)
{
	const IndexInt X = flags.getStrideX(), Y = flags.getStrideY(), Z = flags.getStrideZ();
	const IndexInt idx = flags.index(i, j, k);
	if (flags.isEmpty(idx) && !flags.isOutflow(idx))
	{
		float cnt = 0.;
		if (flags.isFluid(i - 1, j, k)) { pnew[idx] += pressure(i - 1, j, k) * ghostFluidHelper(idx - X, +X, phi, gfClamp); cnt += 1.; }
		if (flags.isFluid(i + 1, j, k)) { pnew[idx] += pressure(i + 1, j, k) * ghostFluidHelper(idx + X, -X, phi, gfClamp); cnt += 1.; }

		if (flags.isFluid(i, j - 1, k)) { pnew[idx] += pressure(i, j - 1, k) * ghostFluidHelper(idx - Y, +Y, phi, gfClamp); cnt += 1.; }
		if (flags.isFluid(i, j + 1, k)) { pnew[idx] += pressure(i, j + 1, k) * ghostFluidHelper(idx + Y, -Y, phi, gfClamp); cnt += 1.; }

		if (flags.is3D()) {
			if (flags.isFluid(i, j, k - 1)) { pnew[idx] += pressure(i, j, k - 1) * ghostFluidHelper(idx - Z, +Z, phi, gfClamp); cnt += 1.; }
			if (flags.isFluid(i, j, k + 1)) { pnew[idx] += pressure(i, j, k + 1) * ghostFluidHelper(idx + Z, -Z, phi, gfClamp); cnt += 1.; }
		}

		if (cnt>0.) { pnew[idx] /= cnt; }
	}
}
// *****************************************************************************
KERNEL(bnd = 1)
void CorrectVelocityFull(FlagGrid& flags, MACGrid& vel, Grid<Real>& pressure)
{
	IndexInt idx = flags.index(i, j, k);
	if (flags.isFluid(idx))
	{
		if (flags.isEmpty(i - 1, j, k) || flags.isFluid(i - 1, j, k)) vel[idx].x -= (pressure[idx] - pressure(i - 1, j, k));
		if (flags.isEmpty(i - 1, j, k) || flags.isFluid(i, j - 1, k)) vel[idx].y -= (pressure[idx] - pressure(i, j - 1, k));
		if (flags.is3D() && (flags.isEmpty(i - 1, j, k) || flags.isFluid(i, j, k - 1))) vel[idx].z -= (pressure[idx] - pressure(i, j, k - 1));
	}
...
PYTHON() void correctVelocities(MACGrid& vel, Grid<Real>& pressure, FlagGrid& flags, Grid<Real>* phi = 0, Real gfClamp = 1e-04, bool mergeGhostFluid = false)
{
	if (mergeGhostFluid)
	{
		// correct everywhere
		CorrectVelocityFull(flags, vel, pressure);
	}
	else
	{
		correctVelocity(vel, pressure, flags, 1e-3,	phi);

		//correctVelocity(flags, vel, pressure);
		// if (phi) {
		// 	CorrectVelocityGhostFluid(vel, flags, pressure, *phi, gfClamp, nullptr, 0.0);
		// 	// improve behavior of clamping for large time steps:
...
//// TEST Pressure Solve without Vel Correction
...
//! surfTens: surface tension coefficient
//! retRhs: return RHS divergence, e.g., for debugging; optional
PYTHON() void solvePressureOnly(MACGrid& vel, Grid<Real>& pressure, FlagGrid& flags, Real cgAccuracy = 1e-3,
	Grid<Real>* phi = 0,
	...
	bool mergeGhostFluid = false)
{
	...
	if (mergeGhostFluid) {
		// store ghost fluid pressure in grid
		Grid<Real> pnew(parent);
		pnew.copyFrom(pressure);
		ComputePressureGhostFluid(vel, flags, pressure, pnew, *phi, gfClamp, curv, surfTens);
		pressure.swap(pnew);
	}
...
Real get_interpolated_neighbors(const Vec3i &pos, const Grid<Real> &pressure, const FlagGrid &flags)
{
	auto ClampVec3 = [](const Vec3i& _In, const Vec3i& _Max) -> Vec3i
	{
		Vec3i New = _In;
		int Min = 0;
		New.x = min(max(New.x, Min), _Max.x - 1);
		New.y = min(max(New.y, Min), _Max.y - 1);
		New.z = min(max(New.z, Min), _Max.z - 1);
		return New;
	};

	const Vec3i CenterCoord = pos;
	const Vec3i Bounds = pressure.getSize();

	const int zStart = pressure.is3D() ? -1 : 0;
	const int zEnd = pressure.is3D() ? 2 : 1;

	int iFluidCellCount = 0;
	Real fInterpolant = 0.0;
	for(int x = -1; x < 2; ++x)
	{
		for(int y = -1; y < 2; ++y)
		{
			...
			{
				Vec3i CurPos = ClampVec3(CenterCoord + Vec3i(x,y,z), Bounds);
				if(flags.isFluid(CurPos))
				{
					fInterpolant += pressure(CurPos);
					++iFluidCellCount;
				}
				else if(flags.isEmpty(CurPos))
				{
					++iFluidCellCount;
				}
			}
	...
	if(iFluidCellCount > 0)
		fInterpolant /= Real(iFluidCellCount);
	else
		fInterpolant = 0.0f;
...
//----------------------------------------------------------------------------------------------------
// update the corner boundary values (for creating smooth pressure fields without solid boundary conditions)
PYTHON() void smoothPressureCorner(Grid<Real> &pressure, const FlagGrid &flags, const int boundary)
{
	if (boundary < 1)
		return;

	Vec3i size = pressure.getSize();
	Vec3i maxIndex = size-1;
	
	Vec3i CurPos;

	if(pressure.is3D())
	{
		for(int x = boundary; x < size.x - boundary; ++x)
		{	
			// x,1,1
			CurPos = Vec3i(x, boundary, boundary);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
			// x,126,1
			CurPos = Vec3i(x, maxIndex.y - boundary, boundary);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
			// x,1,126
			CurPos = Vec3i(x, boundary, maxIndex.z - boundary);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
			// x,126,126
			CurPos = Vec3i(x, maxIndex.y - boundary, maxIndex.z - boundary);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
		}
		for(int y = boundary; y < size.y - boundary; ++y)
		{	
			// 1,y,1
			CurPos = Vec3i(boundary, y, boundary);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
			// 126,y,1
			CurPos = Vec3i(maxIndex.x - boundary, y, boundary);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
			// 1,y,126
			CurPos = Vec3i(boundary, y, maxIndex.z - boundary);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
			// 126,y,126
			CurPos = Vec3i(maxIndex.x - boundary, y, maxIndex.z - boundary);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
		}
		for(int z = boundary; z < size.z - boundary; ++z)
		{	
			// 1,1,z
			CurPos = Vec3i(boundary, boundary, z);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
			// 126,1,z
			CurPos = Vec3i(maxIndex.x - boundary, boundary, z);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
			// 1,126,z
			CurPos = Vec3i(boundary, maxIndex.y - boundary, z);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
			// 126,126,z
			CurPos = Vec3i(maxIndex.x - boundary, maxIndex.y - boundary, z);
			if(flags.isObstacle(CurPos))
				pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
		}
	}
	else
	{
		/// Ground Plane
		// 1,1,0
		CurPos = Vec3i(boundary, boundary, 0);
		pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
		/// 126,1,0
		CurPos = Vec3i(maxIndex.x - boundary, boundary, 0);
		pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);

		/// Top Plane
		// 1,126,0
		CurPos = Vec3i(boundary, maxIndex.y - boundary, 0);
		pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
		// 126,126,0
		CurPos = Vec3i(maxIndex.x - boundary, maxIndex.y - boundary, 0);
		pressure(CurPos) = get_interpolated_neighbors(CurPos, pressure, flags);
	}
}
